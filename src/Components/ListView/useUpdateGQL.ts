...
export function useUpdateGQL( listViewStore:ListViewStore, update?:string ){
  const pageGQLStore = usePageGQLStore();

  const createQueryGQL = ()=>{
    const GQL_STRING = `
      query ($first:Int, $page:Int){
        ${update}(first:$first, page:$page, where:${listViewStore.toWhereGaphiQL()}, orderBy:${listViewStore.toOrderByGraphiQL()}){
          data 
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[listViewStore.rowSchemaStore.fields.size, update, listViewStore.refreshQueryFlag])
